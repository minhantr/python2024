...
class MarkSheet:
    ...
    def calculate_gpa(self, marks, credits):
        return np.dot(marks, credits) / np.sum(credits)

    def sort_students_by_gpa(self):
        self.students.sort(key=lambda student: self.calculate_gpa(list(self.marks[course.id][student.id] for course in self.courses), [3, 3]))  # Assuming each course has a credit of 3, and considering only two courses
